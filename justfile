...
@build:
  npx slidev build --base=/pymi-pyo3
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bpymi-pyo3\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/pymi-pyo3 --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/pymi-pyo3 --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/pymi-pyo3
          break
          ;;
        [Nn])
          break
